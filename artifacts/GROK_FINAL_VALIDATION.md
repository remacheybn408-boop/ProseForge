...
Validation source commit: `09e4fe6`
...
| API unit/integration | `docker compose ... run --rm api-test` | 0 | 576 passed, 1 skipped |
...
| Web typecheck/unit/build | `docker compose ... run --rm web-test` | 0 | 17 files, 46 tests passed; typecheck/build passed |
| Authenticated E2E | `docker compose ... run --rm e2e` | 0 | 11 passed (parallel 4-worker run) |
...
- The Writer assistant has no implemented fold/unfold control yet; it remains visible at narrow widths, but the planned foldable assistant entry point is not verified.
- The Context page shows the model-aware aggregate budget and pin/unpin, but per-item token estimates and priority/exclude/edit/delete controls are not implemented or verified.
