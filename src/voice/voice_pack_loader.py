...
Loads voice pack YAML files from:
  1. packs/templates/voice_pack/  (template/reference packs)
  2. packs/voice/base/            (existing base packs)
...
def _resolve_search_dirs(root: Path, extra_dirs: list[str] = None) -> list[Path]:
    ...
    dirs = []
    # packs/templates/voice_pack/ (template/reference packs)
    templates_dir = root / "packs" / "templates" / "voice_pack"
    if templates_dir.exists():
        dirs.append(templates_dir)
    # packs/voice/base/ (base packs)
    base_dir = root / "packs" / "voice" / "base"
    if base_dir.exists():
