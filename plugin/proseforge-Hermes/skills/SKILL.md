...
核心思路不是"怎么写"，而是"怎么保证写对"——通过 30+ 门禁强制执行写作纪律。
...
{
  "name": "nf_pipeline",
  "description": "写作流水线。action=pre/post/review/batch/volume",
  "required": ["action"],
  "properties": {
    "action":     {"enum": ["pre", "post", "review", "batch", "volume"]},
...
4. `review` 只做审读，不再承诺旧版 `rewrite` 改写路径
