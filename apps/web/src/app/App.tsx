...
function Studio({ project }: { project: Project }) {
  ...
  useEffect(() => { listChapters(project.id).then(items => { setChapters(items); setChapter(items[0] ?? null); setMessage(items.length ? "Ready to write" : "Import an outline to create chapters"); }).catch(() => setMessage("Unable to load chapters")); listModelProfiles().then(items => { setProfiles(items); setProfileId(items[0]?.id ?? ""); }).catch(() => undefined); }, [project.id]);
  useEffect(() => { if (!chapter) return; let active = true; const loadVersions = () => listChapterVersions(chapter.id).then(items => { if (!active) return; setVersions(items); const current = items.find(item => item.id === chapter.active_version_id) ?? items.at(-1); if (!content.trim() || current?.content) setContent(current?.content ?? ""); setBaseVersion(current?.version_no); if (items.length) setMessage(`Loaded saved version ${current?.version_no ?? items.at(-1)?.version_no}`); }).catch(() => setMessage("Unable to load the saved chapter")); loadVersions(); const timer = window.setInterval(() => { if (!content.trim()) loadVersions(); }, 1000); return () => { active = false; window.clearInterval(timer); }; }, [chapter, content]);
  const chapterDraftKey = { projectId: project.id, chapterId: chapter?.id ?? "none", draftType: "chapter" as const };
  ...
  const downloadMarkdown = async () => { try { const result = await requestExport(project.id, "md", versions.map(item => item.id)); window.open(result.download_url, "_blank", "noopener,noreferrer"); } catch { setMessage("Export could not be prepared."); } };
  const exportSnapshot = async (payload: { format: "txt" | "md" | "docx" | "epub"; version_ids: string[] }) => { try { const result = await requestExport(project.id, payload.format, payload.version_ids); window.open(result.download_url, "_blank", "noopener,noreferrer"); setExportOpen(false); } catch { setMessage("Export could not be prepared."); } };
  return <section className="studio-layout"><aside className="chapter-tree"><strong>Chapters</strong>{chapters.map(item => <button className={chapter?.id === item.id ? "selected" : ""} key={item.id} onClick={() => setChapter(item)}>{String(item.chapter_no).padStart(2, "0")} · {item.title}</button>)}{!chapters.length && <small>{message}</small>}</aside><div className="editor-pane"><div className="chapter-head"><span>{chapter ? `Chapter ${String(chapter.chapter_no).padStart(2, "0")}` : project.title}</span><span className="status">{message}</span></div><textarea className="editor" value={content} onChange={event => setContent(event.target.value)} placeholder="Your chapter will appear here…" /><button className="primary" onClick={save} disabled={!chapter}>Save version</button><button onClick={downloadMarkdown}>Download Markdown</button><button onClick={() => setExportOpen(true)}>Export snapshot</button>{exportOpen && <div role="dialog" aria-modal="true" aria-label="Export snapshot"><ExportDialog projectId={project.id} versionIds={versions.map(item => item.id)} onExport={exportSnapshot} /><button onClick={() => setExportOpen(false)}>Cancel</button></div>}</div><div className="review-pane chat-pane"><strong>Writing companion</strong>{profiles.length > 0 && <label>Model profile<select value={profileId} onChange={event => setProfileId(event.target.value)}>{profiles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}<div className="chat-messages">{chat.length === 0 && <p>Ask for a scene revision or continuity check.</p>}{chat.map(item => <div className={`chat-message ${item.role}`} key={item.id}>{item.content || "Waiting for the worker…"}<small>{item.status}</small></div>)}</div><div className="chat-composer"><textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder="Ask your companion…" /><button onClick={fork} disabled={!conversation || !chat.length}>Fork branch</button><button className="primary" onClick={send}>Send</button></div></div></section>;
}
...
function SettingsView() {
  ...
  const probe = async (item: Credential) => { setMessage(`Checking ${item.provider}…`); try { await probeProvider(item.provider); setMessage(`${item.provider} connection is healthy.`); } catch { setMessage(`${item.provider} probe failed; check the endpoint or key.`); } };
  const saveProfile = async () => { if (!profileName.trim() || !modelId.trim()) return setMessage("Enter a profile name and model ID."); try { const profile = await saveModelProfile({ name: profileName.trim(), role: profileRole, config: { provider, model: modelId.trim() } }); setProfiles([...profiles, profile]); setProfileName(""); setModelId(""); setMessage(`${profileRole === "writer" ? "Writer" : "Editor"} profile saved.`); } catch { setMessage("Could not save the model profile."); } };
  return <section className="detail-view"><div className="detail-heading"><p className="eyebrow">MODEL SETTINGS</p><h2>Provider connections</h2><p>Connect a provider once. The raw secret is never rendered back into the browser.</p></div><div className="settings-form"><label>Provider<select value={provider} onChange={event => setProvider(event.target.value)}>{["openai", "anthropic", "google", "deepseek", "kimi", "dashscope", "zhipu", "volcengine", "baidu", "tencent", "minimax", "xai", "mistral", "cohere", "ollama", "vllm"].map(item => <option key={item} value={item}>{item}</option>)}</select></label><label>API key<input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} autoComplete="new-password" /></label><label>Base URL (optional)<input value={baseUrl} onChange={event => setBaseUrl(event.target.value)} /></label><button className="primary" onClick={save}>Save provider</button><p className="form-message" aria-live="polite">{message}</p></div><div className="detail-list">{credentials.map(item => <div className="detail-card" key={item.id}><strong>{item.provider}</strong><span>{item.masked_key}</span><span>Configured</span><button onClick={() => probe(item)}>Test connection</button></div>)}</div><div className="settings-form"><h3>Writer / Editor profile</h3><label>Role<select value={profileRole} onChange={event => setProfileRole(event.target.value as "writer" | "editor")}><option value="writer">Writer — drafting</option><option value="editor">Editor — review and rewrite</option></select></label><label>Profile name<input value={profileName} onChange={event => setProfileName(event.target.value)} placeholder="Fast draft writer" /></label><label>Model ID<input value={modelId} onChange={event => setModelId(event.target.value)} placeholder="gpt-4.1-mini" /></label><button onClick={saveProfile}>Save profile</button></div><div className="detail-list">{profiles.map(profile => <div className="detail-card" key={profile.id}><strong>{String(profile.config.role ?? "writer")} · {profile.name}</strong><span>{String(profile.config.provider)} / {String(profile.config.model)}</span></div>)}</div></section>;
}
...
export default function App() {
  ...
  const load = () => { listProjects().then(items => { setProjects(items); const savedId = window.localStorage.getItem("proseforge.current-project"); const restored = items.find(item => item.id === savedId) ?? items[0] ?? null; setProject(current => current ?? restored); if (restored) { window.localStorage.setItem("proseforge.current-project", restored.id); setView("studio"); } setAuthenticated(true); }).catch(() => setAuthenticated(false)); };
  useEffect(() => { loadRuntimeConfig().then(() => Promise.all([getHealth(), Promise.resolve(load())])).then(() => setConnection("Online")).catch(() => { setConnection("Offline"); setAuthenticated(false); }); }, []);
