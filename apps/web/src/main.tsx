...
import { AppQueryProvider, useHealthQuery, useProjectsQuery, queryClient } from "./app/query";
...
function ContextView({ project }: { project: Project }) {
  ...
  return <section className="detail-view"><div className="detail-heading"><p className="eyebrow">{t("context")}</p><h2>{t("contextHero")}</h2><p>{t("contextIntro")}</p><p className="context-usage">Used tokens: {used.toLocaleString()}</p></div><div className="settings-form"><label>{t("addMemory")}<textarea value={content} onChange={event => setContent(event.target.value)} placeholder={t("addMemoryPlaceholder")} /></label><button className="primary" onClick={add}>{t("addContext")}</button></div><div className="detail-list">{items.map(item => <div className="detail-card" key={item.id}><div><strong>{item.pinned ? "📌 " : ""}{item.source_type}</strong><span>{item.content}</span></div><button onClick={() => pin(item)}>{item.pinned ? t("unpin") : t("pin")}</button></div>)}</div></section>;
}
...
function App() {
  ...
  useEffect(() => {
    if (projectsQuery.isSuccess) {
      ...
      if (restored && route.view === "projects") {
