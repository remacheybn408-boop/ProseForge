...
function SettingsView() {
  ...
  return <section className="detail-view"><div className="detail-heading"><p className="eyebrow">MODEL SETTINGS</p><h2>Provider connections</h2><p>Connect a provider once. The raw secret is never rendered back into the browser.</p></div><div className="settings-form"><label>Provider<select value={provider} onChange={event => setProvider(event.target.value)}><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option><option value="google">Google Gemini</option><option value="ollama">Ollama</option></select></label><label>API key<input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} autoComplete="new-password" /></label><label>Base URL (optional)<input value={baseUrl} onChange={event => setBaseUrl(event.target.value)} /></label><button className="primary" onClick={save}>Save provider</button><p className="form-message" aria-live="polite">{message}</p></div><div className="detail-list">{credentials.map(item => <div className="detail-card" key={item.id}><strong>{item.provider}</strong><span>{item.masked_key}</span><span>Configured</span></div>)}</div></section>;
}
