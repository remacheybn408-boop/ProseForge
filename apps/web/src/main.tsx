...
function Studio({ project }: { project: Project }) {
  ...
  const [chapters, setChapters] = useState<Chapter[]>([]); const [chapter, setChapter] = useState<Chapter | null>(null); const [content, setContent] = useState(""); const [baseVersion, setBaseVersion] = useState<number | undefined>(); const [message, setMessage] = useState(t("loadingChapters"));
  ...
  useEffect(() => { if (!chapter) return; let active = true; const loadVersions = () => listChapterVersions(chapter.id).then(items => { if (!active) return; setVersions(items); const current = items.find(item => item.id === chapter.active_version_id) ?? items.at(-1); if (!content.trim() || current?.content) setContent(current?.content ?? ""); setBaseVersion(current?.version_no); setDiff([]); if (items.length) setMessage(`Loaded saved version ${current?.version_no ?? items.at(-1)?.version_no}`); }).catch(() => setMessage("Unable to load the saved chapter")); loadVersions(); const timer = window.setInterval(() => { if (!content.trim()) loadVersions(); }, 1000); return () => { active = false; window.clearInterval(timer); }; }, [chapter, content]);
  const chapterDraftKey = { conversationId: `chapter:${chapter?.id ?? "none"}`, branchId: "editor", draftType: "chapter" as const };
  ...
  const restore = async (version: ChapterVersion) => { if (!chapter) return; try { await activateChapterVersion(chapter.id, version.id); setContent(version.content); setBaseVersion(version.version_no); setMessage(`Restored version ${version.version_no}`); } catch { setMessage("Could not restore that version"); } };
...
function App() {
  ...
  return <div className="shell"><aside className="rail"><div className="brand">P<span>F</span></div><nav>{nav("projects", t("projects"))}{nav("studio", t("writingStudio"))}{nav("outline", t("outlineIntake"))}{nav("context", t("context"))}{nav("workflow", t("workflow"))}{nav("settings", t("settings"))}</nav><div className="rail-bottom"><div className="language-switcher" aria-label="Language"><button className={language === "zh-CN" ? "selected" : ""} onClick={() => setLanguage("zh-CN")}>{t("languageChinese")}</button><span>/</span><button className={language === "en-US" ? "selected" : ""} onClick={() => setLanguage("en-US")}>{t("languageEnglish")}</button></div><span>{connection === "Online" ? t("apiOnline") : connection === "Offline" ? t("apiOffline") : t("connectionChecking")}</span></div></aside><main className="main"><header><div><p className="eyebrow">{t("currentProject")}</p><h1>{project?.title ?? t("projects")}</h1></div>{project && <button className="ghost" onClick={() => setView("projects")}>{t("allProjects")}</button>}</header>{view === "projects" && <Projects projects={projects} onOpen={item => { setProject(item); window.localStorage.setItem("proseforge.current-project", item.id); setView("studio"); }} onCreated={item => { setProjects([...projects, item]); setProject(item); window.localStorage.setItem("proseforge.current-project", item.id); setView("outline"); }} />}{project && view === "studio" && <><Studio project={project} /><ProjectVersionHistory project={project} /></>}{project && view === "outline" && <OutlineView project={project} onWorkflow={item => { setWorkflow(item); setView("workflow"); }} />}{project && view === "context" && <ContextView project={project} />}{project && view === "workflow" && <WorkflowView project={project} workflow={workflow} onWorkflow={setWorkflow} />}{view === "settings" && <SettingsView />}</main><aside className="inspector"><section><h3>{t("projectStatus")}</h3><p>{project ? t("readyToContinue") : t("noProjects")}</p><small>{t("dockerSaved")}</small></section><section><h3>{t("workflow")}</h3><p>{workflow ? workflow.status : t("notStarted")}</p><button className="link" onClick={() => setView("workflow")}>{t("openWorkflow")}</button></section></aside></div>;
}
