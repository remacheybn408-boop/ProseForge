...
## 3. 改写闭环输出（rewrite / accept）

`nf_pipeline action=rewrite` 生成改写卡，Agent 写改稿，`action=accept` 出对比（详见 REVISION_LOOP.md）：

```
outputs/rewrite_cards/
└── chapter_NNN_rewrite_card.md       ← 改写卡（rewrite 产出）

<chapters 目录>/
└── chapter_NNN_revised.txt           ← 改稿（Agent 写）

exports/reports/
├── chapter_NNN_revision_tasks.json   ← 修改任务（rewrite 产出）
├── chapter_NNN_rewrite_log.json      ← 改动记录（accept 产出）
└── chapter_NNN_revision_diff_report.json  ← 改前/改后对比（accept 产出）
```
